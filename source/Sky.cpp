...
void Sky::advance(double dt)
{
	...
	{
		rainAnimation += dta;
		from = to = 4 + floor(fmod(rainAnimation, 1) * 2);
		progress = 0;
	}
	...
	if (rainyDay)
	{
		if (game->time.checkHour(8))
			rainSound.Play(game);
		if (game->time.checkHour(16))
			rainSound.Stop();
	}
...
void Sky::render(sf::RenderTarget &target) const
{
	...
	for (int x = cmin.x; x <= cmax.x; x++)
	{
		for (int y = cmin.y; y <= cmax.y; y++)
		{
			...
			cloud.setTexture(app->bitmaps[c]);
			...
			for (int i = 0; i < 2; i++)
			{
				...
				// cloud.setScale(w, h);
				cloud.setOrigin({(float)w / 2, (float)h / 2});
				cloud.setColor(sf::Color(255, 255, 255, 255 * (i == 0 ? 1.0 : progress)));
				cloud.setPosition({(float)position.x, (float)position.y});
				target.draw(cloud);
				game->drawnSprites++;
			}
		}
